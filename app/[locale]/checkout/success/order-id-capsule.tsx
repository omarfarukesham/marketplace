...
const OrderIdCapsule = ({ orderSequenceId, index }: { orderSequenceId: number; index: number }) => {
  return (
    <div key={orderSequenceId} className='flex items-center gap-10 rounded border px-3 py-2'>
      ...
      <Copy
        onClick={() => navigator.clipboard.writeText(orderSequenceId.toString()).then(() => toast.success('Id Copied'))}
