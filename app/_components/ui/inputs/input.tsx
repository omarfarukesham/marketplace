...
export const InputField = forwardRef<HTMLInputElement, ComponentProps<'input'>>(({ className, ...rest }, ref) => {
  return <input className={merge('rounded-md border border-gray-600 px-4 py-3', className)} ref={ref} {...rest} />;
});
  ...
  ({ label, error, containerClassName, className, extraInfo, ...rest }, ref) => {
    return (
      <div className={merge('grid gap-2', containerClassName)}>
        ...
        <InputField {...rest} ref={ref} className={merge({ '!border-danger outline-danger': error }, className)} />
        {!error && extraInfo && (
